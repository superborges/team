...
`, 'local-verification/project-template.xlsx', 'local-verification/project-browser-check.xlsx', code, date]);
  ...
  await expect(page.locator('.import-preview')).toContainText('待处理 1 行');
  const applyResponse = page.waitForResponse(response => response.url().endsWith(`/imports/${preview.id}/apply`));
  page.once('dialog', dialog => dialog.accept());
  await page.getByRole('button', { name: '确认导入有效行', exact: true }).click();
  ...
  await expect(page.locator('.import-preview')).toContainText('已导入 1 行');
  const retryResponse = page.waitForResponse(response => response.url().endsWith(`/imports/${preview.id}/apply`));
  page.once('dialog', dialog => dialog.accept());
  await page.getByRole('button', { name: '重新检查并导入', exact: true }).click();
  ...
  await expect(page.getByRole('button', { name: '重新检查并导入', exact: true })).toBeEnabled();
