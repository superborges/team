...
function categoryLabel(entry: DraftEntry) { const type = entryType(entry); return type === 'PROJECT' ? '项目' : type === 'NON_PROJECT' ? '非项目' : type === 'IDLE' ? '待分配' : entry.workItemId ? '类别待确认' : '待选对象'; }
function weekHint(value: Day) { if (value.onsite && !value.totalMinutes) return '现场日'; if (!value.enrolled || !value.requiredMinutes && !value.totalMinutes) return '无要求'; if (!value.totalMinutes) return '未填'; return value.validation.ready ? '已说明' : '待完善'; }
...
<template>
  <main class="mobile-time">
    <div class="mobile-title">
      <div><h1>我的工时</h1><p>按日记录实际投入，按自然周送审。</p></div>
    ...
    <div v-if="conflict" class="conflict-note">当前输入已保留。请核对后重新读取，系统不会覆盖其他修改。<VanButton size="small" @click="refresh">重新读取此日期</VanButton></div>
    <div v-if="loading && !day" class="mobile-empty" role="status">正在读取日记录…</div>
    ...
    <template v-if="day">
      ...
      <section class="mobile-day" :aria-busy="loading" aria-label="当日概览">
        ...
        <div class="mobile-totals">
          <div data-summary="required"><span>应填</span><strong>{{ hours(day.requiredMinutes) }}<small>h</small></strong></div>
          <div data-summary="explained"><span>已说明</span><strong>{{ total }}<small>h</small></strong></div>
          <div data-summary="remaining" :class="{ shortage: total * 60 < day.requiredMinutes }"><span>尚差</span><strong>{{ Math.max(0, day.requiredMinutes / 60 - total) }}<small>h</small></strong></div>
        </div>
        <div class="day-completeness">
          <div v-if="day.requiredMinutes > 0" class="explanation-track" role="progressbar" aria-label="时数说明完整度" :aria-valuenow="explanationProgress" :aria-valuetext="`应填 ${hours(day.requiredMinutes)} 小时，已说明 ${total} 小时`" :aria-valuemin="0" :aria-valuemax="100"><span :style="{ width: `${explanationProgress}%` }"></span></div>
          <p>{{ day.requiredMinutes === 0 ? '当日无最低填报要求' : total * 60 >= day.requiredMinutes ? '已达到当日应填时数，送审仍需通过业务校验。' : '请据实补充尚未说明的时间。' }}</p>
        </div>
        <div class="mobile-breakdown" aria-label="时间构成">
          ...
          <div data-summary="idle"><span>待分配</span><strong>{{ hours(idle * 60) }}<small>h</small></strong></div>
          <div data-summary="leave"><span>请假</span><strong>{{ hours(day.leaveMinutes) }}<small>h</small></strong></div>
        </div>
        <div class="summary-caption"><p><span>实际工作 {{ actual }}h</span><span v-if="unclassifiedHours > 0">（含待确认分类 {{ hours(unclassifiedHours * 60) }}h）</span><span v-else>（项目工作＋非项目工作）</span></p><p>填报口径含草稿与当前输入；请假仅展示出勤，不计入已说明。</p></div>
        <p class="day-approval"><span>审批进度</span><strong>{{ reviewProgress(day) || '暂无送审记录' }}</strong><small v-if="day.periodStatus !== 'OPEN'">{{ day.periodStatus === 'CLOSED' ? '月份已封账' : '已到封账时点' }}</small></p>
        <p v-if="!day.editable" class="read-only">{{ !day.enrolled ? '当前日期不在填报适用区间内。' : '该日期不可编辑，可能已到封账时点。' }}</p>
      </section>
      <section class="mobile-onsite" aria-labelledby="onsite-title">
        ...
        <VanCheckbox aria-label="当天在项目现场" :model-value="!!onsite" :disabled="!day.editable || day.onsite?.editable === false || day.onsite?.action === 'CANCEL' || busy" @update:model-value="toggleOnsite">当天在项目现场</VanCheckbox>
        <p class="onsite-help">含项目交通与必要驻留，独立记录 1 个自然日；实际工时另行填报。</p>
        <template v-if="onsite">
          <label for="onsite-project">现场归属项目</label>
          ...
          <VanField v-model="onsite.reason" type="textarea" rows="2" label="现场事由" placeholder="说明交通、驻留或实施工作" maxlength="200" :disabled="!day.editable || day.onsite?.editable === false || day.onsite?.action === 'CANCEL' || busy" />
          <label v-if="!onsite.id && correctionLinks.length">关联现场更正<select v-model="onsite.correctionRequestId" :disabled="busy"><option :value="null">不关联</option><option v-for="c in correctionLinks.filter(c => c.kind === 'ONSITE')" :key="c.id" :value="c.id">{{ c.date }} · {{ c.workItemName }} #{{ c.id }}</option></select></label>
        </template>
        <div id="h5-onsite-actions" class="onsite-actions"></div>
      </section>
      <div class="records-heading"><h2>当天记录 <small>{{ entries.length }} 条</small></h2><button type="button" :disabled="!day.editable || busy" @click="copyPrevious">{{ copying ? '正在读取…' : '复制上一工作日' }}</button></div>
      <div v-if="!entries.length" class="mobile-empty"><h3>当天还没有记录</h3><p>{{ day.requiredMinutes === 0 ? '当日无最低填报要求；有实际投入时，可据实添加。' : '添加项目工作、部门事务，或据实确认待分配时间。' }}</p><VanButton size="small" type="primary" plain :disabled="!day.editable || busy" @click="addEntry">添加第一条记录</VanButton></div>
      <article v-for="(entry, index) in entries" :key="entry.id ?? `new-${index}`" class="mobile-entry">
        ...
        <label :for="`entry-${index}-item`">归集对象</label>
        <select :id="`entry-${index}-item`" v-model="entry.workItemId" :disabled="!day.editable || !entry.editable || busy" @change="changeObject(entry)"><option disabled value="">选择项目或非项目对象</option><option v-for="item in catalog.workItems" :key="item.id" :value="item.id">{{ item.name }}{{ item.type === 'IDLE' ? '（待分配）' : '' }}</option></select>
        <div class="mobile-entry-pair">
          <label :for="`entry-${index}-kind`">类型<select :id="`entry-${index}-kind`" v-model="entry.kind" :disabled="!day.editable || !entry.editable || entry.kind === 'IDLE' || busy"><option v-if="entry.kind === 'IDLE'" value="IDLE">待分配</option><option value="WORK">实际工作</option><option value="TRAVEL">项目交通</option></select></label>
        ...
        <VanField v-model="entry.content" :label="entry.kind === 'IDLE' ? `记录 ${index + 1} 待分配说明` : `记录 ${index + 1} 工作内容`" type="textarea" rows="3" autosize maxlength="200" show-word-limit :aria-label="`记录 ${index + 1} 工作内容`" :placeholder="entry.kind === 'IDLE' ? '暂无任务安排' : '10–200 字，记录实际完成的工作'" :disabled="!day.editable || !entry.editable || busy" />
        <p v-if="Number(entry.hours) > 8" class="field-hint">单条超过 8 小时，请核对。</p>
        <VanField v-if="actual * 60 > (day.redFlagMinutes??960) && entry.kind !== 'IDLE'" v-model="entry.redReason" label="超长说明" placeholder="实际工作超过提示阈值，请说明原因" maxlength="200" :disabled="!day.editable || !entry.editable || busy" />
      </article>
      ...
      <p class="mobile-footnote">填写完整与审批通过分别显示。通过项需先申请更正。</p>
      ...
      <SubmissionPanel class="mobile-submission" :day="day" :dirty="dirty" :disabled="busy" onsite-target="#h5-onsite-actions" @busy="processing = $event" @refresh="load()">
        <template #save-status><span :class="{ saved: !!saved }" aria-live="polite">{{ dirty ? '有未保存的修改' : saved || '已读取最新记录' }}</span></template>
