...
<template><div class="workflow-page"><div class="workflow-head"><div><h1>我的通知</h1><p>查看与你有关的填报、审批和更正提醒。</p></div><button :disabled="busy" @click="load">刷新</button></div><p v-if="error" class="workflow-error" role="alert">{{ error }}</p><div v-if="!rows.length" class="workflow-empty">{{ busy?'正在加载…':'暂无通知。' }}</div><article v-for="item in rows" :key="item.id" class="workflow-panel"><div class="workflow-between"><strong>{{ item.status==='LOCAL'?'站内通知':'业务提醒' }}</strong><small>{{ displayTime(item.createdAt) }}</small></div><p class="preserve-lines">{{ item.body }}</p><RouterLink v-if="internalPath(item.path)" :to="internalPath(item.path)!">查看关联记录</RouterLink></article></div></template>
