...
<template>
  <div v-if="!me" class="login-page">
    <div class="login-story"><div class="brand-mark">时</div><h1>每一天的投入，<br>清楚记录。</h1><p>记录每天的工作时间，了解项目投入和人员工作量。</p><div class="login-line">工时与项目成本管理系统</div></div>
    <section class="login-form">
      <span class="app-label">工时与投入</span><h2>{{ localLogin ? (demoMode ? '体验演示系统' : '进入测试系统') : '登录系统' }}</h2>
      <p v-if="localLogin" class="muted">当前为{{ demoMode ? '演示' : '本地测试' }}环境，请选择一个测试身份体验。</p>
      ...
      <form v-if="localLogin" @submit.prevent="login">
        ...
        <ElButton type="primary" native-type="submit" :loading="busy" class="full login-submit">进入系统</ElButton>
      </form>
      <p v-else-if="!busy" class="muted">{{ status?.ssoConfigured ? '请在企业微信中打开公司的工时应用。' : '企业微信登录暂未开通，请联系管理员。' }}</p>
  ...
  <div v-else class="workspace">
    ...
    <aside class="sidebar">
      <nav aria-label="主导航"><RouterLink to="/time"><ElIcon aria-hidden="true"><Clock /></ElIcon>我的工时</RouterLink><RouterLink to="/approvals"><ElIcon aria-hidden="true"><Checked /></ElIcon>审批中心</RouterLink><RouterLink to="/notifications"><ElIcon aria-hidden="true"><Bell /></ElIcon>我的通知</RouterLink><RouterLink v-if="!me.canManage && me.roles.includes('DEPARTMENT_MANAGER')" to="/department-items"><ElIcon aria-hidden="true"><FolderOpened /></ElIcon>非项目事项</RouterLink><p class="nav-section">分析与月报</p><RouterLink to="/reports/workload"><ElIcon aria-hidden="true"><DataAnalysis /></ElIcon>人员工时分析</RouterLink><RouterLink v-if="canReadCosts" to="/reports/costs"><ElIcon aria-hidden="true"><Coin /></ElIcon>项目投入与成本</RouterLink><RouterLink to="/reports/onsite"><ElIcon aria-hidden="true"><FolderOpened /></ElIcon>现场统计</RouterLink><RouterLink to="/periods"><ElIcon aria-hidden="true"><Document /></ElIcon>月度结算</RouterLink><RouterLink v-if="me.roles.some(r => ['ADMIN','PM','DEPARTMENT_MANAGER','LEADER'].includes(r))" to="/rates"><ElIcon aria-hidden="true"><Coin /></ElIcon>成本标准</RouterLink><template v-if="me.canManage"><p class="nav-section">基础管理</p><RouterLink to="/people"><ElIcon aria-hidden="true"><User /></ElIcon>人员与组织</RouterLink><RouterLink to="/work-items"><ElIcon aria-hidden="true"><FolderOpened /></ElIcon>项目与事项</RouterLink><RouterLink to="/imports"><ElIcon aria-hidden="true"><Upload /></ElIcon>数据导入</RouterLink><RouterLink to="/coverage"><ElIcon aria-hidden="true"><Checked /></ElIcon>数据完整性核查</RouterLink><RouterLink to="/settings"><ElIcon aria-hidden="true"><Calendar /></ElIcon>系统设置</RouterLink></template></nav>
      <div class="sidebar-foot">以实际工作为准<br>草稿保存后可继续完善</div>
    </aside>
    <div class="main-shell">
      <div v-if="status?.mode === 'local'" class="environment-note"><span class="status-dot"></span>{{ demoMode ? '演示环境 · 测试数据' : '本地测试环境 · 测试数据' }}</div>
      <ElAlert v-if="error" :title="error" type="error" :closable="false" />
      <main :class="{ 'time-workspace': route.path === '/time' }"><div v-if="(route.meta.manage && !me.canManage) || (route.meta.departmentManage && !me.canManage && !me.roles.includes('DEPARTMENT_MANAGER'))" class="empty-state"><h2>暂无此页面的访问权限</h2><p>如需使用此功能，请联系管理员开通权限。</p><RouterLink to="/time">返回我的工时</RouterLink></div><RouterView v-else :key="route.path" /></main>
    </div>
  </div>
</template>
